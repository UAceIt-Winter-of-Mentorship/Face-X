...
specs_ori = cv2.imread('Sunglass.png', -1)
...
while 1:
    ...
    for (x, y, w, h) in faces:
        if h > 0 and w > 0:
            Sunglass_symin = int(y + 1.5 * h / 5)
            Sunglass_symax = int(y + 2.5 * h / 5)
            sh_Sunglass = Sunglass_symax - Sunglass_symin
            ...
            face_glass_roi_color = img[Sunglass_symin:Sunglass_symax, x:x + w]
            face_cigar_roi_color = img[cigar_symin:cigar_symax, x:x + w]
            face_mus_roi_color = img[mus_symin:mus_symax, x:x + w]

            specs = cv2.resize(specs_ori, (w, sh_Sunglass), interpolation=cv2.INTER_CUBIC)
